import { Button, Flex, Input, InputGroup, Stack, Text, useToast } from "@chakra-ui/react"
...
export const CreateOrUpdateUser = ({ onClose, user, users, setUsers }) => {
  ...
  const handleUpdateOrCreateUser = useCallback(async () => {
    ...
    if (user.id) {
      try {
        ...
        if (!newUser.email && !newUser.name && !newUser.cpf && newUser.phones[0].ddd.trim() === '' && newUser.phones[0].number.trim() === '') {
          return toast({
    ...
    try {
      ...
      if (!newUser.email && !newUser.name && !newUser.passowrd && !newUser.cpf && newUser.phones[0].ddd.trim() === '' && newUser.phones[0].number.trim() === '') {
        return toast({
          title: `preencha todos os dados!`,
          status: 'warning',
          isClosable: true,
  ...
  return (
    ...
    <Card p='5'>
      <Flex flexDir={'column'}>
        <Text fontWeight={'bold'} fontSize='1.5rem' mb='3'>{user.id ? 'Editar usuário' : 'Criar usuário'}</Text>
        ...
        <Stack spacing={3}>
          ...
          <Flex flexDir={'column'}>
            <Text>Email: </Text>
            <InputGroup bg='#F1F1F1' borderRadius={'8px'} alignItems={'center'} justifyContent='center'>
              ...
              <Input
                ...
                _placeholder={{ opacity: 0.5, color: 'gray.400', fontWeight: 'bold' }}
              />
            </InputGroup>
          </Flex>
        ...
        <Flex w='100%' justify={'space-between'} mt='6'>
          ...
          >{user.id ? 'Confirmar edição' : 'Criar'}</Button>
