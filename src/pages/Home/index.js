    ...
    >
      ...
      >
        ...
        >
          ...
          <Thead >
            <Tr>
              ...
              <Th onClick={() => {
                ...
                <Flex align={'center'} justify='center' gridGap={'5px'} cursor='pointer'>
                  <Text>
                    Adicionar
                  </Text>
                  <AddIcon />
          ...
          <Tbody >
            {users.map((item, index) => item.active && (
              <Tr key={index}>
                <Td >{item.name}</Td>
                <Td >{item.email}</Td>
                <Td >{formatToCPF(item.cpf)}</Td>
                <Td  >{`${item.phones[0]?.ddd ?? '... '} - ${item.phones[0]?.number ?? '...'}`}</Td>
      ...
      <ModalWrapper isOpen={showModal}>
        ...
        <CreateOrUpdateUser
          onClose={() => setShowModal(!showModal)} user={user}
          users={users}
          setUsers={setUsers}
        />
